...
Resolves: {resolution}
Current implied probability: YES ask = {yes_ask:.1%} | NO ask = {no_ask:.1%}
{news_block}{memory_block}

Your task: estimate the true probability this event resolves YES.

Rules:
- The market price reflects crowd wisdom. Only deviate with specific evidence.
- Deviating >10% from market price requires strong justification.
- Be well-calibrated: avoid overconfidence.

Respond with ONLY a JSON object:
{{"p_yes": <float between 0 and 1>, "rationale": "<one concise sentence>"}}"""

    raw = call_openai(prompt, max_tokens=150)
...
def select_top_markets(markets: list, n: int = TOP_N_MARKETS) -> list:
    ...
    logger.info(
        f"Market filter: {len(markets)} total → {len(filtered)} after filter → top {min(n, len(filtered))}"
    )
    return filtered[:n]
...
def run():
    ...
    part = api.upsert_participant(
        experiment_id,
        model="custom:news-gpt4o-kelly-t1",
        rep=0,
